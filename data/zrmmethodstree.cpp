...
{
public:
    explicit mtree_item_delegate(ZrmMethodsTree* mtree): QItemDelegate(mtree), m_methods_tree(mtree) {}
    ...
    if (db.isOpen() && this->m_abstract_methods)
    {
        ...
        for (int i = 0; ret && i < topLevelItemCount(); i++)
        {
            ...
            if (it == table_method && uc.toInt())
            {
                ...
                ret &= ZrmDatabase::write_method_uservals(db, item_id(item), item->data(column_name, role_user_voltage ), item->data(column_name, role_user_capacity ), false);
                if (ret)
...
)
{
    ...
    if (item && item->data(column_name, role_table).toInt() == table_method)
    {
        ...
        QString method_name = model_name.length() ?  QString("%1:%2").arg( model_name, item->text(column_name)) : item->text(column_name) ;
...
QTreeWidgetItem* ZrmMethodsTree::search_method_by_name(const QString& name)
{
    if (m_abstract_methods)
    {
        QList<QTreeWidgetItem*> items = findItems(name, Qt::MatchFlag::MatchExactly);
        return items.count() ? items.first() : nullptr;
    }
    return nullptr;
}
